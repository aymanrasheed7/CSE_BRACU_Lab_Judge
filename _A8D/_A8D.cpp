...
lll maxAi[] = { 0, 10, 10, 1000, 1000000, 1000000000 };
lll oHash[] = { 0, 13157, 12644, 1231737177, 2086034874, 1203637573 };
vector<string> OutputH;
vector<lll> InputN;
vector<vector<pair<lll, lll>>> InputA;
vector<pair<lll, lll>> OutputB;
...
inline void prepareInput() {
    if (batch == 1) {
        InputN = { 6, 5 };
        InputA = { {{1, 3}, {2, 5}, {3, 7}, {4, 6}, {6, 8}, {7, 9}},
        {{1, 4}, {2, 5}, {6, 7}, {4, 8}, {3, 6}} };
    }
    else if (batch == 2) {
        InputN = { 1, 7 };
        ...
        for (test = 0; test < nTest[batch]; ++test) {
            ...
            for (lll i = 0; i < N; ++i) if (InputA[test][i] = make_pair(
                maxAi[batch] - getRandInt(0, N), maxAi[batch] - getRandInt(0,
                    N)), InputA[test][i].second < InputA[test][i].first)
                swap(InputA[test][i].first, InputA[test][i].second);
        }
    }
    ofstream fout(inp);
    for (fout << nTest[batch] << "\n", test = 0; test < nTest[batch]; ++test) {
        fout << InputN[test] << "\n";
        for (lll i = 0; i < InputN[test]; ++i) fout <<
            InputA[test][i].first << " " << InputA[test][i].second << "\n";
    }
    fout.close();
}
...
inline void validateOutput() {
    try {
        OutputH.clear();
        lll i, j = test = -1;
        for (ifstream fin(out); fin >> word; assertThrow(!j)) {
            assertThrow(1 == sscanf(word.c_str(), "%lld%*c", &j));
            for (OutputH.push_back(word), OutputB.resize(i = j),
                assertThrow(nTest[batch] > ++test); i--; assertInputInt(fin,
                    OutputB[i].first), assertInputInt(fin, OutputB[i].second));
            if (j) for (sort(OutputB.begin(), OutputB.end()), i = j; --i;
                assertThrow(OutputB[i - 1].second < OutputB[i].first));
            if (j) for (sort(InputA[test].begin(), InputA[test].end()),
                i = InputN[test]; i-- && j;)
                if (InputA[test][i] == OutputB[j - 1]) --j;
        }
