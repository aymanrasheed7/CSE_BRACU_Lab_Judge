...
inline void prepareInput() {
    if (batch == 1) {
        ...
        InputH.resize(nTest[batch]);
        InputA.resize(nTest[batch]);
        for (test = 0; test < nTest[batch]; ++test) {
            lll& N = InputN[test] = getRandInt(1, maxN[batch]);
            lll& L = InputL[test] = getRandInt(-maxLHAi[batch], maxLHAi[batch]);
            lll& H = InputH[test] = getRandInt(-maxLHAi[batch], maxLHAi[batch]);
            if (L > H) swap(L, H);
            InputA[test].resize(N);
            for (lll i = 0; i < N; ++i)
                InputA[test][i] = getRandInt(-maxLHAi[batch], maxLHAi[batch]);
        }
    }
    ofstream fout(inp);
    for (fout << nTest[batch] << "\n", test = 0; test < nTest[batch]; ++test) {
        fout << InputN[test] << " " <<
            InputL[test] << " " << InputH[test] << "\n";
