...
lll maxM[] = { 0, 10, 10, 30, 3000, 75000 };
lll maxW[] = { 0, 10, 10, 100, 10000, 1000000 };
lll oHash[] = { 0, 13159, 26006, 1557863811, 248821543, 1927243390 };
...
inline void prepareInput() {
    ...
    if (batch == 1) {
        ...
        for (test = 0; test < nTest[batch]; ++test) {
            ...
            InputW[test].resize(M), w = M;
            if ((1 & getRandInt(0, 1)) && 2 < N && 1 < M) --w,
                InputU[test][w] = 1, InputV[test][w] = 2,
                InputW[test][w] = getRandInt(2, maxW[batch] - 1),
                InputE[test].insert(make_pair(1, 2)), --w,
                InputU[test][w] = 2, InputV[test][w] = N,
                InputW[test][w] = InputW[test][w + 1] ^ 1,
