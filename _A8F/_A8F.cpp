...
lll maxN[] = { 0, 3, 3, 20, 5000, 100000 };
lll maxLDi[] = { 0, 15, 15, 100, 10000, 1000000 };
lll oHash[] = { 0, 12898, 2999761, 179422893, 894383339, 1883874566 };
vector<string> OutputH;
vector<lll> InputN;
vector<vector<lll>> InputL, InputD;
...
inline void prepareInput() {
    if (batch == 1) {
        ...
        for (test = 0; test < nTest[batch]; ++test) {
            ...
            InputL[test].resize(N), InputD[test].resize(N);
            for (lll i = 0; i < N; ++i)
                InputL[test][i] = getRandInt(1, maxLDi[batch]),
                InputD[test][i] = getRandInt(1, maxLDi[batch]);
        }
    }
    ofstream fout(inp);
    for (fout << nTest[batch] << "\n", test = 0; test < nTest[batch]; ++test) {
        ...
        for (lll i = 0; i < InputN[test]; ++i)
            fout << InputL[test][i] << " " << InputD[test][i] << "\n";
    }
    fout.close();
}
