...
int nTest[] = { 0, 2, 2, 6000, 20, 2 };
int maxN[] = { 0, 10, 20, 30, 10000, 100000 };
int maxAi[] = { 0, 10, 100, 1000, 1000000, 1000000000 };
...
inline void validateOutput() {
    try {
        ...
        for (test = 0; test < nTest[batch]; ++test) {
            int N = InputN[test], M = 1 << (32 - __builtin_clz(N));
            for (vector<int> A(M, 0); N--; A[i] = stoi(OutputH[k++]))
