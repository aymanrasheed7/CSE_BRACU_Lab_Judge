...
lll maxN[] = { 0, 10, 10, 20, 2000, 100000 };
lll maxM[] = { 0, 10, 10, 30, 3000, 150000 };
lll oHash[] = { 0, 24464, 2984846, 1232783538, 1827574813, 562432105 };
vector<string> OutputH;
vector<lll> InputN, InputM, InputS, InputD, InputK;
vector<vector<lll>> InputU, InputV, OutputP;
...
inline void prepareInput() {
    ...
    if (batch == 1) {
        ...
        for (test = 0; test < nTest[batch]; ++test) {
            ...
            InputS[test] = getRandInt(1, N);
            InputD[test] = getRandInt(1, N);
            InputK[test] = getRandInt(1, N);
...
inline void validateOutput() {
    try {
        ...
        for (ifstream fin(out); fin >> word; ) {
            assertThrow(sscanf(word.c_str(), "%lld%*c", &p) == 1);
            if (q) --q, OutputP[test].push_back(p);
            else assertThrow(nTest[batch] > ++test),
                OutputP[test].clear(), q = p + 1, OutputH.push_back(word);
        }
        ...
        for (test = 0; test < nTest[batch]; ++test)
            if (!OutputP[test].empty()) {
                ...
                for (lll i = OutputP[test].size(); --i;) assertThrow(
