...
int cpp = 1000, java = 2000, py = 2000, nBatch = 3;
int weight[] = { 0, 1, 1, 8 };
int nTest[] = { 0, 1, 3, 100000 };
int maxAB[] = { 0, 10, 100, 1000000000 };
vector<int> InputA, InputB, OutputC;
...
inline void prepareInput() {
    if (batch == 1) {
        InputA = vector<int>({ 2 });
        InputB = vector<int>({ 3 });
        nTest[batch] = InputA.size();
    }
    else if (batch == 2) {
        InputA = vector<int>({ 4, 6, 8 });
        InputB = vector<int>({ 5, -7, 90 });
        nTest[batch] = InputA.size();
    }
    else {
        ...
        for (test = 0; test < nTest[batch]; ++test) {
            InputA[test] = getRandInt(-maxAB[batch], maxAB[batch]);
            InputB[test] = getRandInt(-maxAB[batch], maxAB[batch]);
        }
    }
    ofstream fout("in.txt");
    for (fout << nTest[batch] << '\n', test = 0; test < nTest[batch]; ++test)
        fout << InputA[test] << " " << InputB[test] << '\n';
    fout.close();
}
...
inline void validateOutput() {
    try {
        ...
        for (ifstream fin("out.txt"); fin >> word;)
            OutputC.push_back(stoi(word));
        assertThrow(OutputC.size() == nTest[batch]);
        for (test = 0; test < nTest[batch]; ++test)
            assertThrow(InputA[test] + InputB[test] == OutputC[test]);
    }
    catch (...) {
